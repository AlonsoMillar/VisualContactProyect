...
export class InfoPage implements OnInit {
  ...
  async ngOnInit() {
    ...
    this.route.queryParams.subscribe(async params => {
      ...
      if (c) {
        ...
        this.contact.phone = c.numero.startsWith('569') ? c.numero.slice(3) : c.numero; // Mostrar sin 569 si lo tiene
  ...
  async callContact() {
    ...
    if (!numero.startsWith('+56')) {
      numero = numero.startsWith('9') ? '+569' + numero : '+56' + numero;
    }
      ...
      buttons: [
        { text: 'Cancelar', role: 'cancel' },
        {
          ...
          handler: async (data) => {
            ...
            const numeroOriginal = numero; // guardamos cómo lo ingresó para mostrar
            ...
            if (!/^9\d{8}$/.test(numero) && !/^569\d{8}$/.test(numero)) {
              const toast = await this.toastCtrl.create({ message: 'Número inválido. Debe ser un número chileno válido (9XXXXXXXX o 569XXXXXXXX).', duration: 3000, color: 'warning' });
              await toast.present();
            ...
            // Formatear para guardar internamente en DB
            ...
            // Mostrar en UI tal como lo ingresó
            this.contact.name = nombre;
            this.contact.phone = numeroOriginal;
