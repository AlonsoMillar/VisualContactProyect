...
export class DatabaseService {
  ...
  public async initDB() {
    ...
    try {
      ...
      // Tabla call_logs (versión original)
      await this.dbInstance.executeSql(`
      ...
      // Añadir columna "info" si no existe
      await this.dbInstance.executeSql(`
        ALTER TABLE call_logs ADD COLUMN info TEXT
      `, []).catch(() => {});
  ...
  public async updateUser(user: User) {
    ...
    await this.dbInstance!.executeSql(
      `UPDATE users SET name = ?, email = ?, phone = ?, password = ?, photo = ? WHERE id = ?`,
  ...
  public async addOrUpdateContact(nombre: string, numero: string, destacado = false, photo = '') {
    ...
    try {
      ...
      console.error('Error addOrUpdateContact:', err);
    }
  }
  ...
  public async deleteContact(nombre: string) {
    ...
    await this.dbInstance!.executeSql(
      'DELETE FROM contacts WHERE nombre = ?',
      [nombre]
    );
  ...
  public async addCallLog(contact_name: string, type: 'call' | 'message', info: string | null = null) {
    await this.ensureDBReady();

    const timestamp = new Date().toISOString();

    await this.dbInstance!.executeSql(
      'INSERT INTO call_logs (contact_name, type, timestamp, info) VALUES (?, ?, ?, ?)',
      [contact_name, type, timestamp, info]
    );
  }
